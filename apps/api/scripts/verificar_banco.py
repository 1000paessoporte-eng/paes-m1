...
COMPROBACIONES_CIENCIAS: dict[str, str] = {
    ...
    # Ácido-base
    # --- Quimica: leyes ponderales y balance ---
    "12 g de carbono con 32 g de oxígeno": f"{12 + 32} g",
    "50 g de reactantes. Tras la reacción se recogen 18 g": f"{50 - 18} g",
    "átomos de oxígeno hay en 3Ca(OH)": f"{3 * 2} átomos",
    "Reaccionan 4 g de hidrógeno con 32 g de oxígeno": f"{4 + 32} g",
    "3 g de un elemento A se combinan siempre con 8 g": f"{8 * 6 // 3} g",
    "8 g de oxígeno por cada gramo de hidrógeno": f"{8 * 10} g",
    "reactantes 25 g y 40 g": f"{25 + 40 - 48} g",
    "10 g de un metal con 4 g de oxígeno y sobran 2 g": f"{10 - 2 + 4} g",
    # --- Quimica: mol, masa molar y estequiometria ---
    # Masas atomicas del enunciado: H=1, C=12, N=14, O=16, Na=23, Mg=24,
    # S=32, Cl=35,5, Ca=40, Fe=56, Zn=65.
    "masa molar del agua, $H_2O$": f"{2 * 1 + 16} g/mol",
    "masa molar del carbonato de calcio": f"{40 + 12 + 3 * 16} g/mol",
    "moles de agua hay en 36 gramos de agua": f"{36 // 18} moles",
    "masa tienen 2 moles de cloruro de sodio": f"{2 * (23 + 35.5):.0f} g",
    "hay en 88 gramos de ese gas": f"{88 // 44} moles",
    "ocupan 3 moles de un gas en condiciones normales": f"{3 * 22.4:.1f} litros".replace(".", ","),
    "partir de 4 moles de hidrógeno": f"{4} moles",
    "para obtener 4 moles de amoníaco": f"{3 * 4 // 2} moles",
    "masa molar de la glucosa": f"{6 * 12 + 12 * 1 + 6 * 16} g/mol",
    "átomos de hidrógeno hay en un mol de agua": "1,204 × 10²⁴",
    "dióxido de carbono se obtiene al quemar un mol de metano": f"{12 + 2 * 16} g",
    "oxígeno se consumen al quemar 2 moles de metano": f"{2 * 2} moles",
    "descomponer 4 moles de agua oxigenada": f"{4 // 2} moles",
    "masa molar del ácido sulfúrico": f"{2 * 1 + 32 + 4 * 16} g/mol",
    "moles de hierro hay en una muestra de 5,6": f"{5.6 / 56:.1f} moles".replace(".", ","),
    "forma a partir de 48 gramos de magnesio": f"{(48 // 24) * (24 + 16)} g",
    "partir de 4 gramos de hidrógeno": f"{(4 // 2) * 18} g",
    "descomposición de 200 gramos": f"{(200 // 100) * 44} g",
    "partículas contienen 3 moles": "1,806 × 10²⁴",
    "contiene 44,8 litros de un gas": f"{int(44.8 / 22.4)} moles",
    "masa molar del amoníaco": f"{14 + 3 * 1} g/mol",
    "moles de átomos de oxígeno hay en un mol de $H_2SO_4$": f"{4} moles",
    "producen con 65 gramos de zinc": f"{65 // 65} mol",
    "Dos moles de un gas desconocido pesan 88 gramos": f"{88 // 2} g/mol",
    "masa de oxígeno hay en 90 gramos de agua": f"{(90 // 18) * 16} g",
    "0,25 moles de un compuesto tiene una masa de 20": f"{int(20 / 0.25)} g/mol",
    "libera un mol de carbonato de calcio": f"{22.4:.1f} litros".replace(".", ","),
    "combustión completa de un mol de propano": f"{3 * 44} g",
    "250 gramos de caliza contiene 80%": f"{int(250 * 0.8) // 100 * 44} g",
    "predice 88 gramos de producto": f"{round(100 * 66 / 88)}%",
    "átomos, en total, hay en un mol de $H_2SO_4$": "4,214 × 10²⁴",
    "Partiendo de 2 moles de nitrógeno": f"{2 * 2} moles",
    "Se calientan 25 gramos de sulfato de cobre": f"{int(25 / 250 * 5 * 18)} g",
    # --- Quimica: reactivo limitante y en exceso ---
    "Con 10 panes y 4 láminas de queso": f"{min(10, 4)} sándwiches",
    "6 moles de hidrógeno con 2 moles de oxígeno. ¿Cuántos moles de hidrógeno sobran": f"{6 - 2 * 2} moles",
    "2 moles de nitrógeno con 3 moles de hidrógeno. ¿Cuántos moles de amoníaco": f"{3 // 3 * 2} moles",
    "2 moles de nitrógeno con 3 moles de hidrógeno. ¿Cuánto nitrógeno queda": f"{2 - 3 // 3} mol",
    "3 moles de metano con 4 moles de oxígeno": f"{4 // 2} moles",
    "2 moles de zinc con 2 moles de ácido clorhídrico": f"{2 // 2} mol",
    "4 gramos de hidrógeno con 16 gramos de oxígeno": f"{int(16 / 32 * 2 * 18)} g",
    "48 gramos de magnesio con 16 gramos de oxígeno. ¿Qué masa de óxido": f"{int(16 / 32 * 2 * 40)} g",
    "48 gramos de magnesio con 16 gramos de oxígeno. ¿Qué masa de magnesio": f"{48 - int(16 / 32 * 2 * 24)} g",
    "24 gramos de carbono con 32 gramos de oxígeno": f"{32 // 32 * 44} g",
    "3 moles de sodio con 2 moles de cloro": f"{3} moles",
    "56 gramos de hierro con 64 gramos de azufre. ¿Qué masa de sulfuro": f"{56 + 32} g",
    "56 gramos de hierro con 64 gramos de azufre. ¿Qué masa de azufre": f"{64 - 32} g",
    "4 moles de A con 4 moles de B": f"{4 // 2} moles",
    "1 mol de carbonato con 1 mol de ácido": f"{1 / 2:.1f} moles".replace(".", ","),
    "10 gramos de hidrógeno con 64 gramos de oxígeno": f"{64 // 32 * 2 * 18} g",
    "54 gramos de aluminio con 96 gramos de oxígeno. ¿Qué masa de óxido": f"{2 * 27 + 3 * 16} g",
    "54 gramos de aluminio con 96 gramos de oxígeno. ¿Qué masa de oxígeno sobra": f"{96 - int(2 / 4 * 3 * 32)} g",
    "28 gramos de nitrógeno con 3 gramos de hidrógeno": f"{14 + 3} g",
    "32 gramos de azufre con 64 gramos de oxígeno. ¿Qué masa de dióxido": f"{32 + 32} g",
    "32 gramos de azufre con 64 gramos de oxígeno. ¿Qué masa de oxígeno": f"{64 - 32} g",
    "3 moles de A con 6 moles de B": f"{6 // 3 * 2} moles",
    "Con 20 pernos y 50 tuercas": f"{min(20, 50 // 2)} uniones",
    "24 gramos de magnesio con 3 moles de ácido": f"{22.4:.1f} litros".replace(".", ","),
    "8 gramos de hidrógeno con 32 gramos de oxígeno": f"{8 + 32} g",
    "100 gramos de caliza con 80%": f"{int(1 / 2 * 44)} g",
    "32,5 gramos de zinc": f"{0.5 * 22.4:.1f} litros".replace(".", ","),
    "predice 40 gramos de producto": f"{round(100 * 34 / 40)}%",
    # --- Quimica: analisis porcentual, formula empirica y molecular ---
    "porcentaje en masa de oxígeno en el agua": f"{100 * 16 / 18:.1f}%".replace(".", ","),
    "porcentaje en masa de carbono en el dióxido de carbono": f"{100 * 12 / 44:.1f}%".replace(".", ","),
    "porcentaje en masa de nitrógeno en el amoníaco": f"{100 * 14 / 17:.1f}%".replace(".", ","),
    "porcentaje en masa de calcio": f"{int(100 * 40 / 100)}%",
    "contiene 40% de carbono, 6,7% de hidrógeno": _sub("CH2O"),
    "75% de carbono y 25% de hidrógeno": _sub("CH4"),
    "empírica $CH_3$ y masa molar 30": _sub(f"C{30 // 15}H{3 * (30 // 15)}"),
    "empírica $CH_2$ y masa molar 42": _sub(f"C{42 // 14}H{2 * (42 // 14)}"),
    "empírica $CH_2O$ y masa molar 180": _sub(f"C{180 // 30}H{2 * (180 // 30)}O{180 // 30}"),
    "85,7% de carbono y 14,3% de hidrógeno": _sub("CH2"),
    "empírica CH y masa molar 78": _sub(f"C{78 // 13}H{78 // 13}"),
    "masa de carbono hay en 44 gramos": f"{12} g",
    "2,4 gramos de carbono y 0,6 gramos de hidrógeno": _sub("CH3"),
    "porcentaje en masa de azufre en": f"{100 * 32 / 98:.1f}%".replace(".", ","),
    "porcentaje en masa de hierro en el óxido": f"{int(100 * 112 / 160)}%",
    "27,3% de carbono y 72,7% de oxígeno": _sub("CO2"),
    "hierro se puede obtener de 160 gramos": f"{2 * 56} g",
    "masa correspondiente a la fórmula empírica": f"{12 + 2 * 1 + 16} g/mol",
    "46,7% de nitrógeno y 53,3% de oxígeno": "NO",
    "40% de azufre y 60% de oxígeno": _sub("SO3"),
    "carbono hay en 200 gramos de ese compuesto": f"{int(200 * 0.40)} g",
    "52,2% de carbono, 13% de hidrógeno": _sub("C2H6O"),
    "empírica $CH_2O$ y masa molar 60": _sub(f"C{60 // 30}H{2 * (60 // 30)}O{60 // 30}"),
    "qué porcentaje de la masa corresponde al agua": f"{int(100 * 5 * 18 / 250)}%",
    "92,3% de carbono y 7,7% de hidrógeno": "CH",
    "porcentaje en masa de sodio": f"{100 * 23 / 58.5:.1f}%".replace(".", ","),
    "porcentaje en masa de oxígeno en el ácido sulfúrico": f"{100 * 64 / 98:.1f}%".replace(".", ","),
    "porcentaje en masa de hidrógeno en la glucosa": f"{100 * 12 / 180:.1f}%".replace(".", ","),
    "250 gramos de carbonato de calcio puro": f"{int(250 * 0.40)} g",
    "8,8 gramos de dióxido de carbono y 3,6 gramos de agua": _sub("CH2"),
    "quemar 4,6 gramos de un compuesto": _sub("C2H6O"),
    "quedan 3,2 gramos de sal anhidra": f"{round((5.0 - 3.2) / 18 / (3.2 / 160))} aguas",
    "densidad de 1,25 g/L": _sub(f"C{int(1.25 * 22.4) // 14}H{2 * (int(1.25 * 22.4) // 14)}"),
    # --- Quimica: soluciones y unidades de concentracion ---
    "0,5 moles de soluto y la disolución se completa hasta 2": f"{0.5 / 2:.2f} mol/L".replace(".", ","),
    "58,5 gramos de sal en un litro": f"{58.5 / 58.5:.0f} mol/L",
    "4 gramos de hidróxido de sodio": f"{4 / 40 / 0.5:.1f} mol/L".replace(".", ","),
    "250 mL de una disolución 2 mol/L": f"{2 * 0.25:.1f} moles".replace(".", ","),
    "2 litros de una disolución 0,5 mol/L": f"{0.5 * 2 * 58.5:.1f} g".replace(".", ","),
    "disolución 0,2 mol/L contiene 0,1 moles": f"{0.1 / 0.2:.1f} litros".replace(".", ","),
    "20 gramos de sal en 180 gramos de agua": f"{int(100 * 20 / 200)}%",
    "5 gramos de soluto en 100 mL": f"{int(100 * 5 / 100)}%",
    "25 mL de alcohol en 100 mL": f"{int(100 * 25 / 100)}%",
    "0,5 moles de soluto en 500 gramos de disolvente": f"{0.5 / 0.5:.0f} mol/kg",
    "2 moles de soluto y 8 moles de disolvente": f"{2 / (2 + 8):.1f}".replace(".", ","),
    "40 gramos de hidróxido de sodio": f"{40 / 40 / 2:.1f} mol/L".replace(".", ","),
    "0,5 litros de una disolución 0,2 mol/L": f"{0.2 * 0.5 * 180:.0f} g",
    "400 gramos de una disolución al 15%": f"{int(400 * 0.15)} g",
    "250 gramos de una disolución acuosa al 8%": f"{int(250 * (1 - 0.08))} g",
    "contaminante a 3 ppm": f"{3 * 2} miligramos",
    ...
    "11,7 gramos de cloruro de sodio": f"{11.7 / 58.5 / 0.4:.1f} mol/L".replace(".", ","),
    "muestra de 500 mL contiene 0,04 miligramos": f"{0.04 / 0.5:.2f} ppm".replace(".", ","),
    "46 gramos de etanol": f"{(46 / 46) / ((46 / 46) + (54 / 18)):.2f}".replace(".", ","),
    ...
    # --- Fisica: peso, normal, tension, elastica y roce ---
    "peso de un cuerpo de 12 kg": f"{12 * 10} N",
    "resorte se estira 5 cm al aplicarle una fuerza de 20 N": f"{int(20 / 0.05)} N/m",
    "caja de 20 kg descansa sobre una superficie horizontal": f"{int(0.4 * 20 * 10)} N",
    "caja de 5 kg está en reposo sobre una superficie con coeficiente": f"{int(0.5 * 5 * 10)} N",
    "bloque de 4 kg es arrastrado sobre una superficie horizontal": f"{int((30 - 0.25 * 4 * 10) / 4)} m/s²",
    "resorte cumple la ley de Hooke con constante 250 N/m": f"{int(250 * 0.12)} N",
    "cuerpo de 6 kg cuelga en reposo de una cuerda vertical": f"{6 * 10} N",
    "cuerpo de 3 kg está en reposo sobre una mesa horizontal": f"{3 * 10} N",
    "resorte de constante 500 N/m sostiene un cuerpo colgado": f"{int(500 * 0.06 / 10)} kg",
    "empuja una caja con una fuerza horizontal de 40 N y la caja no se mueve": f"{40} N",
    "cuerpo de 10 kg cuelga de una cuerda dentro de un ascensor": f"{10 * (10 - 3)} N",
    "bloque de 2 kg se desliza sobre una superficie horizontal y se detiene": f"{4 ** 2 / (2 * 8) / 10:.1f}".replace(".", ","),
    "Dos resortes idénticos de constante 200 N/m": f"{int(40 / (200 / 2) * 100)} cm",
    "cuerpo de 8 kg es levantado verticalmente con velocidad constante": f"{8 * 10} N",
    "alguien tira de él hacia arriba con una cuerda que ejerce 30 N": f"{5 * 10 - 30} N",
    # --- Fisica: potencia, consumo e instalacion domiciliaria ---
    "artefacto funciona con 12 V y por él circulan 3 A": f"{12 * 3} W",
    "hervidor de 2.000 W funciona durante media hora": f"{int(2 * 0.5)} kWh",
    "estufa eléctrica de 1.500 W se usa 4 horas diarias": f"${int(1.5 * 4 * 150)}",
    "artefacto de 1.100 W se conecta a 220 V": f"{int(1100 / 220)} A",
    "resistencia de 8 Ω circula una corriente de 3 A. ¿Qué potencia": f"{3 ** 2 * 8} W",
    "computador portátil de 60 W permanece encendido 8 horas": f"{0.06 * 8 * 30:.1f} kWh".replace(".", ","),
    "ampolleta de 60 W se conecta durante 5 horas": f"{60 * 5 * 3600:,} J".replace(",", "."),
    "panel solar entrega 300 W": f"{0.3 * 5:.1f} kWh".replace(".", ","),
    "motor eléctrico consume 500 W y entrega 400 W": f"{400 * 100 // 500} %",
    "lavadora indica un consumo de 0,8 kWh por ciclo": f"${0.8 * 4 * 4 * 150:,.0f}".replace(",", "."),
    "automático de 16 A a 220 V": f"{220 * 16:,} W".replace(",", "."),
    "televisor de 100 W permanece 6 horas encendido": f"{(100 * 6 + 2 * 18) / 1000:.3f} kWh".replace(".", ","),
    "plancha de 1.200 W se usa 20 minutos": f"{1.2 * 20 / 60:.1f} kWh".replace(".", ","),
    "artefacto de 500 W funciona durante 2 horas": f"{int(0.5 * 2)} kWh",
    "cargador de teléfono consume 5 W": f"{5 * 24 / 1000:.2f} kWh".replace(".", ","),
    # --- Fisica: ley de Ohm y circuitos ---
    "corriente de 2 A durante 5 s": f"{2 * 5} C",
    "resistencia de 15 Ω circula una corriente de 4 A": f"{15 * 4} V",
    "resistencia de 40 Ω se conecta a una fuente de 120 V": f"{120 // 40} A",
    "Dos resistencias de 10 Ω y 20 Ω se conectan en serie": f"{60 // (10 + 20)} A",
    "resistencias de 5 Ω y 15 Ω conectadas a 40 V": f"{40 // (5 + 15) * 15} V",
    "batería de 12 V y dos resistencias de 3 Ω en paralelo": f"{int(12 / (3 / 2))} A",
    "resistencia de 20 Ω en paralelo con otra de 5 Ω": f"{int(20 / (20 * 5 / (20 + 5)))} A",
    "tres resistencias de 2 Ω, 3 Ω y 5 Ω a una fuente de 20 V": f"{20 // (2 + 3 + 5) * 3} V",
    "resistencia de 2 Ω en serie con dos de 6 Ω": f"{int(20 / (2 + 6 / 2))} A",
    "amperímetro colocado en la rama principal marca 6 A": f"{6 - 2} A",
    "obtiene 4 V, 6 V y 10 V": f"{4 + 6 + 10} V",
    # --- Historia y economia sin recomprobar hasta 2026-09-04 ---
    "3.000.000 de menores de 15 años": f"{100 * (3 + 1.5) / 9:.0f}%",
    "canasta básica se estima en $60.000": f"${60000 * 4:,} mensuales".replace(",", "."),
    "10.000.000 de personas en edad de trabajar": f"{100 * 6.5 / 10:.0f}%",
    "6.000.000 de personas están ocupadas": f"{100 * 6 / 10:.0f}%",
    "deuda pública de un país es de 90.000 millones": f"{100 * 90 // 300}%",
    "inflación fue de 10% el primer año": f"{round(100 * (1.1 * 1.1 - 1))}%",
    "compra paltas a $1.500 el kilo": f"${(2400 - 1500) * 120 - 18000:,}".replace(",", "."),
    "gasta $400 en insumos por cada pan": f"${600000 + 400 * 2000:,}".replace(",", "."),
    "vende cada pan a $1.000": f"{600000 // (1000 - 400):,} panes".replace(",", "."),
    "IPC de un país pasa de 100 a 106 puntos": f"{106 - 100}%",
    "vende 200 unidades a 3.000 pesos": f"{200 * 3000:,} pesos".replace(",", "."),
    "5 millones ocupadas y 500 mil buscando empleo": f"{100 * 0.5 / 5.5:.1f}%".replace(".", ","),
    "vende 500 unidades a $2.000 cada una": f"{500 * 2000 - 700000:,} pesos".replace(",", "."),
    # --- Lectora: la unica de las seis que se puede recalcular ---
    "Según la boleta, ¿cuál fue el consumo": f"{14451 - 14238} kWh",
    # --- Ciencias que habian quedado sueltas ---
    "4 electrones en su capa externa": "4",
    "frena desde 30 m/s hasta detenerse en 60 m": f"{-(30 ** 2) / (2 * 60):.1f} m/s²".replace(".", ",").replace("-", "−"),
    "sostiene inmóvil en el aire un contenedor de 500 kg": f"{500 * 10:,} N".replace(",", "."),
    "cromosomas tiene un gameto humano": f"{46 // 2}",
    "volumen ocupa un mol de cualquier gas": f"{22.4:.1f} litros".replace(".", ","),
    "masa tiene un mol de átomos de carbono": f"{12} gramos",
    "cloruro de calcio, $CaCl_2$, se separan sus iones": f"{1 + 2} moles",
    "En la primera etapa hay 1 mol de nitrógeno": f"{2} moles",
    "cuánto debe sumar el porcentaje de todos los elementos": "100%",
    "cuánto suman las fracciones molares": "1",
}
...
COMPROBACIONES: dict[str, str] = {
    ...
    "22 practican fútbol, 16 practican vóleibol": f"{22 + 16 - 8}/40",
    "probabilidad de que ocurra un evento es p": "1 − p",
    ...
    "Se lanza un dado común dos veces": "(1/6) · (1/6)",
    "el número sea par O mayor que 7": f"{5 + 3 - 2}/10",
    ...
    "hay r bolitas rojas y a bolitas azules": "r / (r + a)",
    ...
    "jardín cuadrado de 7 metros de lado se cerca": f"{4 * 7} m de malla y {7**2} m² de pasto",
    "perímetro de un cuadrado de lado 9 cm": f"{4 * 9} cm",
    "se quiere alfombrar completa": "a · b",
    "cinta alrededor de un cuadro rectangular": "2p + 2q",
    "triángulo equilátero de lado 12 cm": f"{3 * 12} cm",
    "avanza la bicicleta en una vuelta completa": "2 · π · r",
    "terreno con forma de trapecio tiene bases": "(B + b) · h / 2",
    "afirma que su área es 96 cm²": f"{12 * 8 // 2} cm²",
    "tres terrenos rectangulares": f"{28 * 20} m²",
    "baldosas cuadradas de 50 cm de lado": str(round(6.5 * 4 / 0.5**2)),
    ...
    "tabla rígida que puede pasar por ella": "√(a² + b²)",
    ...
    "termina en (3, 4) tras aplicarle el vector": f"({3 + 5}, {4 - 7})",
    ...
    "A todos los datos de un conjunto se les suma 4": f"La media aumenta en {4}",
    "ventas diarias de una semana": "15",
    "promedio 5,0 en 5 notas y quiere subirlo a 5,5": f"{6 * 5.5 - 5 * 5.0:.1f}".replace(".", ","),
    ...
    # --- Combinatoria y probabilidad sin recomprobar hasta 2026-09-04 ---
    "ordenar 4 libros diferentes en un estante": str(factorial(4)),
    "comité de 3 personas a partir de un grupo de 6": str(comb(6, 3)),
    "ordenar 9 fotos diferentes en un álbum": f"{factorial(9):,}".replace(",", "."),
    "repartir 2 premios distintos": str(factorial(6) // factorial(4)),
    "probabilidad de que un evento ocurra es 3/5": "2/5",
    "helados con 3 sabores y 2 tipos de cono": str(3 * 2),
    "letras de la palabra \"AMOR\"": str(factorial(4)),
    "carrera de 3 personas": str(factorial(3)),
    "letras de la palabra \"ANA\"": str(factorial(3) // factorial(2)),
    "letras de la palabra \"MAMA\"": str(factorial(4) // (factorial(2) * factorial(2))),
    "2 hombres de un grupo de 5 hombres": str(comb(5, 2) * comb(4, 2)),
    "resultado fue par. ¿Cuál es la probabilidad de que haya sido un 6": "1/3",
    "NO puede ser la probabilidad de un evento": "1,4",
    "Se saca una al azar y resulta azul": "5/9",
    "40% de los estudiantes practica deporte": f"{100 * 15 / 40:.1f}%".replace(".", ","),
    "2% de los productos tiene un defecto": f"{2 * 30 / 100:.1f}%".replace(".", ","),
    "desviación estándar de los datos: 3, 5, 7": f"{sqrt(8 / 3):.2f}".replace(".", ","),
    "todos los datos de un conjunto son iguales": "0",
    # --- Numeros, algebra y geometria sin recomprobar hasta 2026-09-04 ---
    "NO representa el precio final de un producto de $40.000": "40.000 − 20",
    "cuántas horas h han pasado si la población es 64 veces": "= 64",
    "su área es 5 metros cuadrados": f"{5} m",
    "caminar 300 m al norte y después 400 m al este": f"{300 + 400 - int(sqrt(300 ** 2 + 400 ** 2))} m",
    ...
    "f(x) = x³ cuando x = −2": str((-2) ** 3),
    "f(x) = x⁵, ¿cuál es el valor de f(−1) + f(2)": str((-1) ** 5 + 2 ** 5),
    "cuadrado de área 9 cm² se somete a una homotecia de razón k=3": f"{9 * 3 ** 2} cm²",
    "coseno 0,8. Si la hipotenusa mide 15 cm": f"{int(0.8 * 15)} cm",
    "tangente de 1,5": f"{int(20 * 1.5)} m",
    "x = 2y y x + y = 12": str(12 // 3),
