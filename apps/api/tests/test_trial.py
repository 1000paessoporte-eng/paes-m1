...
def test_el_ensayo_del_dia_manda_solo_con_los_limites_activos(
    register_user, db_session, monkeypatch
) -> None:
    """El muro del plan Gratis: con LIMITES_ACTIVOS solo rinde el ensayo del
    día; sin él, arma los ensayos que quiera.
    ...
    try:
        ...
        get_settings.cache_clear()
        assert service.solo_ensayo_del_dia(db_session, user.id) is True

        monkeypatch.delenv("LIMITES_ACTIVOS", raising=False)
        get_settings.cache_clear()
        assert service.solo_ensayo_del_dia(db_session, user.id) is False
    finally:
        monkeypatch.delenv("LIMITES_ACTIVOS", raising=False)
        get_settings.cache_clear()
