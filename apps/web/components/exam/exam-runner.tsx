...
export function ExamRunner() {
  ...
  const flush = useCallback(
    (questionId: number, selectedAlternativeId: number | null) => {
      const now = Date.now();
      ...
      setElapsedByQuestion((prev) => {
        const total = (prev[questionId] ?? 0) + Math.max(0, delta);
        answerExamQuestion(id, questionId, selectedAlternativeId, total).catch(() => {
  ...
  const doSubmit = useCallback(async () => {
    const id = attemptIdRef.current;
    if (id == null) return;
    if (currentQuestion) flush(currentQuestion.id, selections[currentQuestion.id] ?? null);
    try {
  ...
  }, [currentQuestion, selections, flush]);

  // Resumir un intento en curso al montar (estado persistente ante refresh).
  useEffect(() => {
  ...
  if (phase === "idle" || phase === "error") {
    return (
      <StartScreen onStart={handleStart} errorMsg={errorMsg} />
    );
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-surface px-4 py-3">
        ...
        {confirmingSubmit ? (
          <div className="flex items-center gap-2 text-sm">
            ...
            <button
              onClick={doSubmit}
              className="rounded-md bg-danger px-3 py-1.5 font-medium text-white"
            >
              Sí, finalizar
            </button>
            <button
              onClick={() => setConfirmingSubmit(false)}
              className="rounded-md border border-border px-3 py-1.5 text-muted hover:bg-surface-hover"
            >
              Cancelar
            </button>
          </div>
  ...
  return (
    <div className="mx-auto flex max-w-lg flex-col items-center rounded-2xl border border-border bg-surface px-6 py-16 text-center">
      ...
      <p className="mt-2 max-w-sm text-sm text-muted">
        Simulacro cronometrado de 2 horas 20 minutos, con las 32 preguntas del
        banco actual. Tu progreso se guarda automáticamente: puedes recargar
  ...
  return (
    <div className="mx-auto flex max-w-lg flex-col items-center rounded-2xl border border-border bg-surface px-6 py-16 text-center">
      ...
      <p className="mt-6 max-w-sm text-xs text-muted">
        La revisión detallada por pregunta (autopsia del error) estará
        disponible en Smart Feedback.
      </p>
    </div>
  );
}
